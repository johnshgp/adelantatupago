...
class DianDocument(models.Model):
    ...
    document_id = fields.Many2one('account.move', string="Número de documento", readonly=True, required=True)
    ...
    date_document = fields.Date(string="Fecha documento", readonly=True, related='document_id.invoice_date')
    ...
    def _get_resolution_dian(self, data_header_doc):
        ...
        if rec_active_resolution:
            ...
            dict_resolution_dian['InvoiceID'] = data_header_doc.name                              # Codigo del documento
            #13FEB dict_resolution_dian['ContingencyID'] = data_header_doc.contingency_invoice_number 
            dict_resolution_dian['ContingencyID'] = data_header_doc.name     # Número de fcatura de contingencia
    ...
    def request_validating_dian(self, document_id):
        ...
        data_header_doc = self.env['account.move'].search([('id', '=', dian_document.document_id.id)])
        ...
        if dic_result_verify_status['result_verify_status'] == False:
            ...
            if document_type == 'c':
                ...
                for by_validate_cn in by_validate_credit_notes:
                    invoice_validated = self.env['account.move'].search([('name', '=', by_validate_cn.document_id.invoice_origin),('move_type', '=', 'out_invoice'),('state_dian_document', '=', 'exitoso')])
            ...
            for doc_send_dian in docs_send_dian:
                data_header_doc = self.env['account.move'].search([('id', '=', doc_send_dian.document_id.id)])
                dian_constants = self._get_dian_constants(data_header_doc)
    ...
    def enviar_email(self, data_xml_document, invoice_id, fileName):
        ...
        rs_invoice = self.env['account.move'].sudo().search([('id', '=', invoice_id)])
        ...
        if plantilla_correo:
            ...
            plantilla_correo.sudo().send_mail(rs_invoice.id, force_send = True)
    ...
    def _get_dian_constants(self, data_header_doc):
        ...
        dian_constants['ProviderID'] = partner.vat     if partner.vat else ''   # ID Proveedor de software o cliente si es software propio
        ...
        dian_constants['SoftwareSecurityCode'] = self._generate_software_security_code(company.software_identification_code, 
                                                company.software_pin,data_header_doc.name)            # Código de seguridad del software: (hashlib.new('sha384', str(self.company_id.software_id) + str(self.company_id.software_pin)))
        ...
        dian_constants['SupplierID'] = partner.vat if partner.vat else ''       # Identificador fiscal: En Colombia, el NIT
        dian_constants['SupplierSchemeID'] = self.return_number_document_type(partner.l10n_co_document_code)
    ...
    def _generate_data_constants_document(self, data_header_doc, dian_constants, document_type, in_contingency_4):
        ...
        # Generar nombre del archvio xml
        data_constants_document['FileNameXML'] = self._generate_xml_filename(data_resolution, NitSinDV, data_header_doc.move_type, data_header_doc.debit_origin_id)
        data_constants_document['FileNameZIP'] = self._generate_zip_filename(data_resolution, NitSinDV, data_header_doc.move_type, data_header_doc.debit_origin_id)
        ...
        data_constants_document['DebitNoteTypeCode'] = self._get_doctype(data_header_doc.move_type, data_header_doc.debit_origin_id, in_contingency_4)                                 # Tipo de Factura, código: facturas de venta, y transcripciones; tipo = 1 para factura de venta 
        ...
        if self.return_number_document_type(data_header_doc.partner_id.l10n_co_document_code) == 31:
            data_constants_document['IDAdquiriente'] = data_header_doc.partner_id.vat if data_header_doc.partner_id.vat else ''
            data_constants_document['SchemeNameAdquiriente'] = self.return_number_document_type(data_header_doc.partner_id.l10n_co_document_code)
        ...
        data_constants_document['CustomerID'] = data_header_doc.partner_id.vat if data_header_doc.partner_id.vat else '' # Identificador fiscal: En Colombia, el NIT
        data_constants_document['CustomerSchemeID'] = self.return_number_document_type(data_header_doc.partner_id.l10n_co_document_code)                                    # tipo de identificdor fiscal 
        ...
        if dian_constants['Currency'] == data_header_doc.currency_id.id:
            ...
            data_constants_document['CalculationRate'] = self._get_rate_date(data_header_doc.company_id.id,data_header_doc.currency_id.id,data_header_doc.invoice_date)
        ...
        for line_term_pago in data_header_doc.invoice_payment_term_id.line_ids:
            if line_term_pago.days == 0:
                ...
                data_constants_document['PaymentDueDate'] = data_header_doc.invoice_date_due
        ...
        if data_constants_document['InvoiceTypeCode'] in ('91','92'):
            ...
            invoice_cancel = self.env['account.move'].search([('name', '=', data_header_doc.invoice_origin),('move_type', '=', 'out_invoice'),('state_dian_document', '=', 'exitoso')])
            if invoice_cancel:
                ...
                if dian_document_cancel:
                    ...
                    data_constants_document['InvoiceReferenceDate'] = invoice_cancel.invoice_date
        # Datos contingencia
        if data_constants_document['InvoiceTypeCode'] == ('03'):
            data_constants_document['ContingencyReferenceID'] = data_header_doc.contingency_invoice_number
            data_constants_document['ContingencyIssueDate'] = data_header_doc.invoice_date
        ...
        data_constants_document['DescriptionDebitCreditNote'] = data_header_doc.narration
    ...
    def _caculate_TaxExclusiveAmount(self, invoice_id):
        ...
        """
        amount_untaxed = 0.00
        data_lines_doc = self.env['account.move.line'].search([('move_id', '=', invoice_id), ('product_id', '!=', None),('exclude_from_invoice_tab', '=', False)])
        for data_line_doc in data_lines_doc:
            if data_line_doc.tax_ids:
                if not data_line_doc.tax_ids.amount:
                    continue
...
<rep:Document>%(Document)s</rep:Document>
    ...
    def _get_taxs_data(self, invoice_id):
        ...
        data_tax_detail_doc = self.env['account.move.line'].search([('move_id', '=', invoice_id), ('tax_line_id', '!=', None)])

        if data_tax_detail_doc:
            for item_tax in data_tax_detail_doc:
                iva_01 = item_tax.price_subtotal if item_tax.tax_line_id.tax_group_fe == 'iva_fe' else 0.0
                ica_03 += item_tax.price_subtotal if item_tax.tax_line_id.tax_group_fe == 'ica_fe' else 0.0
                inc_04 += item_tax.price_subtotal if item_tax.tax_line_id.tax_group_fe == 'ico_fe' else 0.0 # Mod impuesto
                ret_06 = item_tax.price_subtotal if item_tax.tax_line_id.tax_group_fe == 'ret_fe' else 0.0  # Mod impuesto

                tax_percentage_iva_01 = self.env['account.tax'].search([('id', '=', item_tax.tax_line_id.id)]).amount if item_tax.tax_line_id.tax_group_fe  == 'iva_fe' else tax_percentage_iva_01
                tax_percentage_ica_03 = self.env['account.tax'].search([('id', '=', item_tax.tax_line_id.id)]).amount if item_tax.tax_line_id.tax_group_fe  == 'ica_fe' else tax_percentage_ica_03
                tax_percentage_inc_04 = self.env['account.tax'].search([('id', '=', item_tax.tax_line_id.id)]).amount if item_tax.tax_line_id.tax_group_fe  == 'ico_fe' else tax_percentage_inc_04
                tax_percentage_ret_06 = self.env['account.tax'].search([('id', '=',item_tax.tax_line_id.id)]).amount if item_tax.tax_line_id.tax_group_fe == 'ret_fe' else tax_percentage_ret_06

                invoice_lines = self.env['account.move.line'].search([('move_id', '=', invoice_id), ('tax_ids', 'in', item_tax.tax_line_id.id)])
                for invoice_line in invoice_lines:
                    total_base_iva_01 += invoice_line.price_subtotal if item_tax.tax_line_id.tax_group_fe  == 'iva_fe' else 0
                    total_base_ica_03 += invoice_line.price_subtotal if item_tax.tax_line_id.tax_group_fe  == 'ica_fe' else 0
                    total_base_inc_04 += invoice_line.price_subtotal if item_tax.tax_line_id.tax_group_fe  == 'ico_fe' else 0
                    total_base_ret_06 += invoice_line.price_subtotal if item_tax.tax_line_id.tax_group_fe == 'ret_fe' else 0

                if iva_01 != 0.00 and total_base_iva_01 == 0.00 and item_tax.tax_id.tax_group_fe == 'iva_fe':
                    total_base_iva_01 = item_tax.move_id.amount_untaxed

                if ica_03 != 0.00 and total_base_ica_03 == 0.00 and item_tax.tax_line_id.tax_group_fe == 'ica_fe':
                    total_base_ica_03 = item_tax.move_id.amount_untaxed

                if inc_04 != 0.00 and total_base_inc_04 == 0.00 and item_tax.tax_line_id.tax_group_fe == 'ico_fe':
                    total_base_inc_04 = item_tax.move_id.amount_untaxed

                if ret_06 != 0.00 and total_base_ret_06 == 0.00 and item_tax.tax_line_id.tax_group_fe == 'ret_fe':
                    total_base_ret_06 = item_tax.move_id.amount_untaxed

                if iva_01:
                    dic_taxs_data['iva_' + str(iva_lines)] = self._complements_second_decimal_total(iva_01)
                    dic_taxs_data['tax_percentage_iva_' + str(iva_lines)] = self._complements_second_decimal_total(tax_percentage_iva_01)
                    dic_taxs_data['total_base_iva_' + str(iva_lines)] = self._complements_second_decimal_total(total_base_iva_01)
                    iva_lines += 1

                if ret_06:

                    dic_taxs_data['ret_' + str(ret_lines)] = self._complements_second_decimal_total(ret_06 * -1 if ret_06 < 0 else ret_06)
                    dic_taxs_data['tax_percentage_ret_' + str(ret_lines)] = self._complements_second_decimal_total(tax_percentage_ret_06 * -1 if tax_percentage_ret_06 < 0 else tax_percentage_ret_06)
    ...
    def _generate_lines_data_xml(self, template_line_data_xml, invoice_id, CurrencyID):
        ...
        for data_line in data_lines_doc:
            ...
            if data_line.price_subtotal:
                ...
                InvoiceLineTaxSubtotal_xml = ''
                TaxRoundingAmount = 0
                for line_tax in data_line.tax_ids:
                    ...
                    ILTaxAmount  = float(self._complements_second_decimal_total((ILTaxAmount * -1 if ILTaxAmount < 0 else ILTaxAmount)))
                    ...
                    ILTaxAmountSubtotal = self._complements_second_decimal_total(((data_line.price_total - data_line.price_subtotal) * -1 if (data_line.price_total - data_line.price_subtotal) < 0 else (data_line.price_total - data_line.price_subtotal)))
                    ILTaxableAmount = self._complements_second_decimal_total((data_line.price_subtotal * -1 if data_line.price_subtotal < 0 else data_line.price_subtotal))
                    ILPercent  = self._complements_second_decimal_total((tax.amount * -1 if tax.amount < 0 else tax.amount))
                    ILID = tax.tributes              
    ...
    def _generate_credit_lines_data_xml(self , template_credit_line_data_xml, invoice_id, CurrencyID):
        ...
        for data_line in data_lines_doc:
            ...
            for line_tax in data_line.tax_ids:
                ...
                ILTaxAmount = self._complements_second_decimal_total(((data_line.price_total - data_line.price_subtotal) * -1 if (data_line.price_total - data_line.price_subtotal) < 0 else (data_line.price_total - data_line.price_subtotal)))
                ILTaxableAmount = self._complements_second_decimal_total((data_line.price_subtotal * -1 if data_line.price_subtotal < 0 else data_line.price_subtotal))
                ILPercent  = self._complements_second_decimal_total((tax.amount * -1 if tax.amount < 0 else tax.amount))
                ILID = tax.tributes              
                ILName = tributes[tax.tributes]
            ...
            data_credit_note_line_xml += (template_credit_line_without_taxtotal if ILID == '06' or ILPercent == '0.00'  else template_credit_line_data_xml) % {'ILLinea' : ILLinea,
                                                    'ILInvoicedQuantity' : ILInvoicedQuantity,
                                                    'ILLineExtensionAmount' : ILLineExtensionAmount, # Ojo descuentos mas recargos de la linea
                                                    'ILAmount' : ILAmount,
                                                    'ILDescription' : ILDescription,
    ...
    def _get_lines_invoice(self, invoice_id):
        ...
        move_lines = self.env['account.move.line'].search([('move_id', '=', invoice_id), ('product_id', '!=', None),('exclude_from_invoice_tab', '=', False)])
    ...
    def _second_decimal_total(self, amount):
        if amount:
            return str('{0:.2f}'.format(str(amount)))
        else:
            return 0
